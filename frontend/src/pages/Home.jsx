  ...
  return (
    <div className=" bg-primary-light dark:bg-primary-dark w-full p-1  overflow-hidden transition-colors duration-150">
      ...
      >
        ...
        {isAuthenticated ? (
          <HoverArrowBtn children={"Go to Dashboard"} desti={"/dashboard"} />
        ) : (
          <HoverArrowBtn children={"get started"} desti={"/auth/login"} />
        )}
        <motion.div
          whileHover={{ rotate: 6 }}
          transition={{ duration: 0.3, ease: "backOut" }}
          className="w-14  h-26 mt-4"
        >
          <img
            className="w-full h-full object-fit dark:invert"
            src={arrowpng}
            alt=""
          />
        </motion.div>
